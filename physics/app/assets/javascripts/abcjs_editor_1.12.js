...
window.ABCJS.parse.Parse = function() {
    ...
    var o = new window.ABCJS.parse.tokenizer();
    ...
    var c = function(C) {
        var B = window.ABCJS.parse.gsub(C, "\x12", " ");
        B = window.ABCJS.parse.gsub(B, "&", "&amp;");
        B = window.ABCJS.parse.gsub(B, "<", "&lt;");
        return window.ABCJS.parse.gsub(B, ">", "&gt;")
    };
    ...
    var v = new window.ABCJS.parse.ParseHeader(o, y, z, f);
    ...
    var i = function(B, D) {
        ...
        if (E !== undefined) {
            ...
            if (window.ABCJS.parse.detect(q, function(F) {
                    return (E === F)
                })) {
                return [1, E]
            } else {
                if (!window.ABCJS.parse.detect(z.ignoredDecorations, function(F) {
                        return (E === F)
                    })) {
        ...
        switch (B.charAt(D)) {
            ...
            case "+":
                ...
                if (window.ABCJS.parse.detect(t, function(F) {
                        if (C[1] === F[0]) {
                            C[1] = F[1];
                            return true
                        } else {
    ...
    var n = function(C, I) {
        ...
        var E = false;
        var B = function(J) {
            var K = window.ABCJS.parse.strip(I.substring(G, J));
            G = J + 1;
            if (K.length > 0) {
                if (E) {
                    K = window.ABCJS.parse.gsub(K, "~", " ")
                }
                var L = I.charAt(J);
                if (L !== "_" && L !== "-") {
                    L = " "
        ...
        for (var D = 0; D < I.length; D++) {
            switch (I.charAt(D)) {
                ...
                case "-":
                    if (!B(D) && H.length > 0) {
                        window.ABCJS.parse.last(H).divider = "-";
                        H.push({
                            skip: true,
                            to: "next"
                        })
        ...
        var F = false;
        window.ABCJS.parse.each(C, function(K) {
            if (H.length !== 0) {
                if (H[0].skip) {
                    switch (H[0].to) {
                        case "next":
    ...
    var d = function(C, I) {
        if (!C) {
            y("Can't add symbols before the first line of music", C, 0);
            return
        }
        I = window.ABCJS.parse.strip(I);
        if (I.charAt(I.length - 1) !== "-") {
    ...
    function w() {
        ...
        C.clef = z.currentVoice && z.staves[z.currentVoice.staffNum].clef !== undefined ? window.ABCJS.parse.clone(z.staves[z.currentVoice.staffNum].clef) : window.ABCJS.parse.clone(z.clef);
        C.key = window.ABCJS.parse.parseKeyVoice.deepCopyKey(z.key);
        window.ABCJS.parse.parseKeyVoice.addPosToKey(C.clef, C.key);
        if (z.meter !== null) {
            if (z.currentVoice) {
                window.ABCJS.parse.each(z.staves, function(D) {
                    D.meter = z.meter
                });
                                ...
                                if (K !== null) {
                                    ...
                                    if (L.charAt(S) === " ") {
                                        ...
                                        if (S < L.length && L.charAt(S) === "]") {
                                            ...
                                            if (z.inTie) {
                                                window.ABCJS.parse.each(D.pitches, function(Y) {
                                                    Y.endTie = true
                                                });
                                                z.inTie = false
                                            }
    ...
    this.parse = function(H, F) {
        ...
        H = H.replace(/\\([ \t]*)(%.*)*\n/g, D);
        var C = H.split("\n");
        if (window.ABCJS.parse.last(C).length === 0) {
            C.pop()
        }
        try {
            window.ABCJS.parse.each(C, function(J) {
                if (F) {
                ...
                if (z.is_in_history) {
                    ...
                    if (z.inTextBlock) {
                        if (window.ABCJS.parse.startsWith(J, "%%endtext")) {
                            f.addText(z.textBlock);
                            z.inTextBlock = false
                        } else {
                            if (window.ABCJS.parse.startsWith(J, "%%")) {
                        ...
                        if (z.inPsBlock) {
                            if (window.ABCJS.parse.startsWith(J, "%%endps")) {
                                z.inPsBlock = false
                            } else {
                                z.textBlock += " " + J
                            }
...
(function() {
    ...
    window.ABCJS.parse.parseDirective.addDirective = function(O) {
        var X = function(t, ai) {
        ...
        var D = function(aj) {
            var p = {};
            var t = window.ABCJS.parse.last(aj);
            if (t.type === "number") {
            ...
            if (aj.length > 0) {
                var ai = "";
                window.ABCJS.parse.each(aj, function(ak) {
                    if (ak.token !== "-") {
                        if (ai.length > 0) {
                            ai += " "
                        }
        ...
        switch (ab) {
            ...
            case "scale":
                j = "";
                window.ABCJS.parse.each(M, function(p) {
                    j += p.token
                });
                ae = parseFloat(j);
                if (isNaN(ae) || ae === 0) {
            ...
            case "text":
                ...
                c.addText(window.ABCJS.parse.parseDirective.parseFontChangeLine(Y));
                break;
            case "center":
...
ABCJS.write.Layout.prototype.printNote = function(L, d, S) {
    ...
    if (L.lyric !== undefined) {
        var K = "";
        window.ABCJS.parse.each(L.lyric, function(c) {
...
window.ABCJS.Editor = function(a, b) {
    ...
    this.engraverparams = b.render_options || {};
    if (b.gui) {
...
window.ABCJS.Editor.prototype.renderTune = function(a, g, h) {
    ...
    var c = new ABCJS.write.Printer(f, this.engraverparams);
    c.printABC(b)
};
window.ABCJS.Editor.prototype.modelChanged = function() {
    ...
    this.engraver_controller = new ABCJS.write.Printer(c, this.engraverparams);
    ...
    if (ABCJS.midi.MidiWriter && this.mididiv) {
        ...
        b.addListener(this.engraver_controller);
...
window.ABCJS.Editor.prototype.updateSelection = function() {
    ...
    try {
        this.engraver_controller.rangeHighlight(a.start, a.end)
    } catch (b) {}
};
